...
public class Game extends JPanel{
	...
	public void init() {
		...
		paddle = new Paddle(Frame.WIDTH/2-50, 550, 100, 30);
		balls = new Ball[3];
		balls[0] = new Ball(paddle.getX()+paddle.width/2-12, paddle.getY()-paddle.height/2-10, 23, 23, false, false);
		...
		for(int i = 0; i < gridPos.length; i++) {
			for(int j = 0; j < gridPos[0].length; j++) {
				bricks[count] = new Brick(j*49, i*25, 50, 25, gridPos[i][j]);
				count++;
			}
		}
	}
			...
			for(int i = 0; i < balls.length; i++) {
				if(balls[i] != null) {
					...
					if(balls[i].getY() > Frame.HEIGHT-50) {
						if(totalBallCount <= 1) {
							hasStarted = false;
							balls[i] = new Ball(paddle.getX()+paddle.width/2-12, paddle.getY()-paddle.height/2-10, 23, 23, false, false);
							livesLeft -= 1;
			...
			for(int i = 0; i < bricks.length; i++) {
				...
				if(bricks[i].dropPowerup) {
					for(int j = 0; j < powerups.length; j++) {
						if(powerups[j] == null) {
							powerups[j] = new Powerup(bricks[i].getX(), bricks[i].getY(), 25, 25, bricks[i].hasPowerup());
					...
					if(paddle.isColliding(powerups[i])) {	
						if(powerups[i].powerup == 1) { // add another ball
							for(int j = 0; j < balls.length; j++) {
								if(balls[j] == null) {
									balls[j] = new Ball(Frame.WIDTH/2, 350, 23, 23, false, false);
									totalBallCount++;
									break;
								}
							}
